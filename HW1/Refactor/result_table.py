# Student: Grace Pham - ntp33 - Course: CS 260
# Script Purpose: Creating a dataframe with result lists generated by running 12 functions (in function.py)
# on different values of x in x_list & Visualizing the dataframe to get the Result table


import pandas as pd

'''
Instruction on installing pandas library:
- Open terminal
- Install pip (if not installed)
- Run command: "pip install pandas" on terminal
'''

from results import *

# Create a dataframe from 12 result lists generated by running functions fx with input x_list
df = pd.DataFrame({
    'f1': f1_result,
    'f2': f2_result,
    'f3': f3_result,
    'f4': f4_result,
    'f5': f5_result,
    'f6': f6_result,
    'f7': f7_result,
    'f8': f8_result,
    'f9': f9_result,
    'f10': f10_result,
    'f11': f11_result,
    'f12': f12_result,
}, index=x_list)

# Print Report Table
print(df)
